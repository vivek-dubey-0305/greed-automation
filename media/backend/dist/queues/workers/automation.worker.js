...
import { campaigns, automationEvents, platformPosts, automationRuns } from '../../db/schema';
...
export const automationWorker = new Worker('automation', async (job) => {
    ...
    try {
        ...
        // Call AI
        const generated = await aiProvider.generateCampaignContent(details.campaign.instruction || '', details.platforms.map(p => p.platform), details.media.length);
        // Save platform posts
        for (const p of details.platforms) {
            const pData = generated[p.platform];
            if (pData) {
