...
import { platformPosts, publishAttempts, campaigns, campaignPlatforms, automationRuns, automationEvents, socialAccounts } from '../../db/schema';
...
async function checkCampaignCompletion(campaignId: string) {
  ...
  for (const plat of platforms) {
    ...
    for (const p of pPosts) {
      if (p.status !== PlatformPostStatus.PUBLISHED) allPublished = false;
      if (p.status === PlatformPostStatus.FAILED) anyFailed = true;
    }
  }
