...
import { campaigns, platformPosts, automationRuns, automationEvents } from '../db/schema';
...
export class AutomationService {
   ...
   */
  static async approvePlatformPost(postId: string, forceFailTest = false) {
    const posts = await db.select().from(platformPosts).where(eq(platformPosts.id, postId));
    if (!posts.length) throw new AppError({ message: 'Post not found', code: 'NOT_FOUND', category: 'NotFoundError' });
    const post = posts[0];
  ...
  static async retryPlatform(postId: string) {
    const posts = await db.select().from(platformPosts).where(eq(platformPosts.id, postId));
    if (!posts.length) throw new AppError({ message: 'Post not found', code: 'NOT_FOUND', category: 'NotFoundError' });
    const post = posts[0];
